...
class App:
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_UP, 15, 6) or pyxel.btnp(pyxel.GAMEPAD1_BUTTON_DPAD_UP, 15, 6):
            self.cursor_index = max(self.cursor_index - 1, 0)

        if pyxel.btnp(pyxel.KEY_DOWN, 15, 6) or pyxel.btnp(pyxel.GAMEPAD1_BUTTON_DPAD_DOWN, 15, 6):
    ...
    def draw(self):
        ...
        for i, key in enumerate([key for key in metadata.keys() if key not in ["name", "filepath"]]):
            pyxel.text(
                13,
                90 + ROW_HEIGHT * i,
